  ...
  \begin{enumerate}
    ...
    \item Establish interface for creating a collection of labels
      \begin{itemize}[label=$\multimapdotinv$, leftmargin=3mm]
        ...
        \item Said labels represent possible areas where information is contingent
      \end{itemize}
...
% Zero Technical Debt Concerns
{
  ...
  \begin{itemize}[label=\blitzb]
    \small
    \item Safe syncing or relating to concrete, personal data
    \item Reflecting already updated services
  \end{itemize}
}
...
% Decision Process
{
  \begin{itemize}
    \small
    \item \textbf{Labels} and \textbf{Details} consolidated into top-level \textbf{\textsl{Organization}}
      \begin{itemize}[label=\blitzb]
        \small
        \item Holds contents such as \textsl{OrgDesc, OrgLink, MemRecord}
      \end{itemize}
    \item Armed w/out abstract members to avoid wasteful gold-plating on current requirements
      \normalsize
  \end{itemize}
}
